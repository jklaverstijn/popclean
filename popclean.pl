...
my $VERSION = sprintf "%d.%02d", q$Revision: 1.31 $ =~ m/ (\d+) \. (\d+) /xg;
...
sub printversion {
   ...
   my $PVERSION =  $pop->Version();
   # Also print the version of Mail::POP3Client.
   &logmsg ("info", "Using Mail::POP3Client version $PVERSION\n");
}
...
sub Pop3Maint {
   ...
   for ($i = 1; $i <= $pop->Count; $i++) {
      ...
      # Take date from Date header. It's not our first choice and may even be missing,
      # but it may be needed for later fallback.
      if (defined($date=$header->get("Date"))) {
	      chomp ($date);
	 ...
	 $label_size=' (!)'; 
      ...
      if ($delflag eq "N") {
         ...
         $thisbox_size_kept+=$size;
      }
   } # End of loop over all messages
   $size_kept+=$thisbox_size_kept;
   $kept=$pop->Count-$delcount;
   $total_kept+=$kept;
   $total_delcount+=$delcount;

   $thisbox_size_kept=sprintf("%.0f", $thisbox_size_kept/1024);
   &logmsg ('info', "${username} at $mailserver: $delcount deleted, $kept kept ($thisbox_size_kept KB)");
...
sub parse_config_file {
   ...
   for($i=0; $i<@toks; $i++) {
	 ...
	 # Future: filter out unsupported server prorotcols like IMAP
         $poll->{'proto'} = $toks[++$i]->[0];
      }
      elsif ($t->[0] eq 'port') {
         $poll->{'port'} = $toks[++$i]->[0];
...
sub Connect2POP {
   ...
   while ($attempts--) {
      if ($opt_ssl) {
         ...
         # but USESSL is supported in POP3Client starting at 2.14.
         # This way non-ssl users can work with pre-2.14 versions.
